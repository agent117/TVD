...
public class chestClick implements IPath{
	...
	public static boolean onClick(Player p, Location loc) {
		Inventory inv;
		if(containsLocation(loc) && (inv = getInventory(loc)) != null ) {
			p.openInventory(inv);
			return true;
		}
	...
	public static void createInventories() {
		...
		YamlConfiguration cfg = configManager.getChestConfig();
		int count = 0;
		ItemStack[] items;
		int[] chances;
		...
		List<String> itemList = cfg.getStringList(chestItemListPath);
		...
		for(Location l : locs) {
			...
			for(int i = 0; i < configManager.getChestConfig().getInt(maxChestItems); i++) {
